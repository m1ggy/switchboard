import { zodResolver } from '@hookform/resolvers/zod';
import { useMutation, useQueryClient } from '@tanstack/react-query';
import {
  CheckCircle2,
  Copy,
  Loader2,
  Search,
  ShieldCheck,
} from 'lucide-react';
import { useState } from 'react';
import { useForm } from 'react-hook-form';
import { z } from 'zod';

import { Button } from '@/components/ui/button';
import { Card, CardContent } from '@/components/ui/card';
import { Checkbox } from '@/components/ui/checkbox';
import {
  Dialog,
  DialogContent,
  DialogDescription,
  DialogFooter,
  DialogHeader,
  DialogTitle,
  DialogTrigger,
} from '@/components/ui/dialog';
import {
  Form,
  FormControl,
  FormField,
  FormItem,
  FormLabel,
  FormMessage,
} from '@/components/ui/form';
import { Input } from '@/components/ui/input';
import { Tabs, TabsList, TabsTrigger } from '@/components/ui/tabs';
import { useTRPC } from '@/lib/trpc';

/**
 * AdminCreateAccountDialog
 * ------------------------
 * Internal tool, server-gated to a single allowed email (superAdminProcedure)
 * — client-side visibility here is UX only, not the security boundary.
 *
 * Two modes:
 * - "New user": creates a brand new Firebase login (own credentials), own
 *   company, own number, end-to-end. No password is set by the admin — a
 *   password-setup link comes back for the admin to send the new user.
 * - "Existing user": attaches a company + number to an account that already
 *   exists, looked up by email.
 */

const phoneNumberSchema = z
  .string()
  .min(1, 'Phone number is required')
  .regex(/^\+[1-9]\d{6,14}$/, 'Use E.164 format, e.g. +15551234567');

const NewUserSchema = z.object({
  email: z.string().email(),
  firstName: z.string().min(1, 'First name is required'),
  lastName: z.string().min(1, 'Last name is required'),
  companyName: z.string().min(2, 'Company name must be at least 2 characters'),
  number: phoneNumberSchema,
  label: z.string().optional(),
  wireWebhooks: z.boolean(),
});
type NewUserValues = z.infer<typeof NewUserSchema>;

const ExistingUserSchema = z.object({
  companyName: z.string().min(2, 'Company name must be at least 2 characters'),
  ownerEmail: z.string().email("Owner's email is required"),
  number: phoneNumberSchema,
  label: z.string().optional(),
  wireWebhooks: z.boolean(),
});
type ExistingUserValues = z.infer<typeof ExistingUserSchema>;

type ResolvedOwner = { uid: string; email: string; displayName: string | null };

type CreatedResult = {
  passwordSetupLink?: string;
  company: { id: string; name: string };
};

export default function AdminCreateAccountDialog() {
  const trpc = useTRPC();
  const qc = useQueryClient();

  const [open, setOpen] = useState(false);
  const [mode, setMode] = useState<'new' | 'existing'>('new');
  const [error, setError] = useState<string | null>(null);
  const [result, setResult] = useState<CreatedResult | null>(null);

  const [resolvedOwner, setResolvedOwner] = useState<ResolvedOwner | null>(null);
  const [lookupError, setLookupError] = useState<string | null>(null);
  const [lookupPending, setLookupPending] = useState(false);

  const newUserForm = useForm<NewUserValues>({
    resolver: zodResolver(NewUserSchema),
    defaultValues: {
      email: '',
      firstName: '',
      lastName: '',
      companyName: '',
      number: '',
      label: '',
      wireWebhooks: true,
    },
    mode: 'onChange',
  });

  const existingUserForm = useForm<ExistingUserValues>({
    resolver: zodResolver(ExistingUserSchema),
    defaultValues: {
      companyName: '',
      ownerEmail: '',
      number: '',
      label: '',
      wireWebhooks: true,
    },
    mode: 'onChange',
  });

  async function handleLookup() {
    const email = existingUserForm.getValues('ownerEmail');
    const valid = await existingUserForm.trigger('ownerEmail');
    if (!valid) return;

    setLookupError(null);
    setResolvedOwner(null);
    setLookupPending(true);

    try {
      const found = await qc.fetchQuery(
        trpc.companies.lookupUserByEmail.queryOptions({ email })
      );
      setResolvedOwner(found);
    } catch (e: any) {
      setLookupError(e?.message || 'Could not find that user.');
    } finally {
      setLookupPending(false);
    }
  }

  const { mutateAsync: createNewAccount, isPending: isCreatingNew } =
    useMutation(trpc.companies.createNewAccount.mutationOptions());

  const { mutateAsync: createForExisting, isPending: isCreatingExisting } =
    useMutation(trpc.companies.createAccountForUser.mutationOptions());

  const isPending = isCreatingNew || isCreatingExisting;

  async function invalidateCompanies() {
    await qc.invalidateQueries({
      queryKey: trpc.companies.getUserCompanies.queryOptions().queryKey,
    });
  }

  async function onSubmitNewUser(values: NewUserValues) {
    setError(null);
    try {
      const created = await createNewAccount({
        email: values.email.trim(),
        firstName: values.firstName.trim(),
        lastName: values.lastName.trim(),
        companyName: values.companyName.trim(),
        number: values.number.trim(),
        label: values.label?.trim() || undefined,
        wireWebhooks: values.wireWebhooks,
      });

      await invalidateCompanies();
      setResult({
        passwordSetupLink: created.passwordSetupLink,
        company: created.company,
      });
      newUserForm.reset();
    } catch (e: any) {
      setError(e?.message || 'Failed to create account. Please try again.');
    }
  }

  async function onSubmitExistingUser(values: ExistingUserValues) {
    setError(null);

    if (!resolvedOwner) {
      setError("Look up the owner's email first.");
      return;
    }

    try {
      const created = await createForExisting({
        companyName: values.companyName.trim(),
        userId: resolvedOwner.uid,
        number: values.number.trim(),
        label: values.label?.trim() || undefined,
        wireWebhooks: values.wireWebhooks,
      });

      await invalidateCompanies();
      setResult({ company: created.company });
      existingUserForm.reset();
      setResolvedOwner(null);
    } catch (e: any) {
      setError(e?.message || 'Failed to create account. Please try again.');
    }
  }

  function resetAndClose() {
    newUserForm.reset();
    existingUserForm.reset();
    setResolvedOwner(null);
    setLookupError(null);
    setError(null);
    setResult(null);
    setOpen(false);
  }

  return (
    <Dialog
      open={open}
      onOpenChange={(next) => {
        if (!next) resetAndClose();
        else setOpen(true);
      }}
    >
      <DialogTrigger asChild>
        <Button variant="outline">
          <ShieldCheck className="mr-2 h-4 w-4" /> Create account (admin)
        </Button>
      </DialogTrigger>

      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          <DialogTitle>Create account</DialogTitle>
          <DialogDescription>
            Provisions a company and attaches an already-purchased number. No
            payment required.
          </DialogDescription>
        </DialogHeader>

        {result ? (
          <Card className="border-0 shadow-none">
            <CardContent className="px-0 space-y-4">
              <div className="flex items-center gap-2 text-sm font-medium text-emerald-600">
                <CheckCircle2 className="h-4 w-4" />
                {result.company.name} created.
              </div>

              {result.passwordSetupLink ? (
                <div className="space-y-2">
                  <div className="text-sm text-muted-foreground">
                    Send this link to the new user so they can set their own
                    password. It's only shown once — copy it now.
                  </div>
                  <div className="flex gap-2">
                    <Input readOnly value={result.passwordSetupLink} />
                    <Button
                      type="button"
                      variant="secondary"
                      onClick={() =>
                        navigator.clipboard.writeText(
                          result.passwordSetupLink!
                        )
                      }
                    >
                      <Copy className="h-4 w-4" />
                    </Button>
                  </div>
                </div>
              ) : null}

              <DialogFooter>
                <Button onClick={resetAndClose}>Done</Button>
              </DialogFooter>
            </CardContent>
          </Card>
        ) : (
          <Card className="border-0 shadow-none">
            <CardContent className="px-0 space-y-5">
              <Tabs value={mode} onValueChange={(v) => setMode(v as any)}>
                <TabsList className="grid w-full grid-cols-2">
                  <TabsTrigger value="new">New user</TabsTrigger>
                  <TabsTrigger value="existing">Existing user</TabsTrigger>
                </TabsList>
              </Tabs>

              {mode === 'new' ? (
                <Form {...newUserForm}>
                  <form
                    onSubmit={newUserForm.handleSubmit(onSubmitNewUser)}
                    className="space-y-5"
                    autoComplete="off"
                  >
                    <FormField
                      control={newUserForm.control}
                      name="email"
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel>Email (their login)</FormLabel>
                          <FormControl>
                            <Input
                              type="email"
                              placeholder="owner@example.com"
                              {...field}
                            />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
                    />

                    <div className="grid grid-cols-2 gap-3">
                      <FormField
                        control={newUserForm.control}
                        name="firstName"
                        render={({ field }) => (
                          <FormItem>
                            <FormLabel>First name</FormLabel>
                            <FormControl>
                              <Input placeholder="Jane" {...field} />
                            </FormControl>
                            <FormMessage />
                          </FormItem>
                        )}
                      />
                      <FormField
                        control={newUserForm.control}
                        name="lastName"
                        render={({ field }) => (
                          <FormItem>
                            <FormLabel>Last name</FormLabel>
                            <FormControl>
                              <Input placeholder="Doe" {...field} />
                            </FormControl>
                            <FormMessage />
                          </FormItem>
                        )}
                      />
                    </div>

                    <FormField
                      control={newUserForm.control}
                      name="companyName"
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel>Company name</FormLabel>
                          <FormControl>
                            <Input placeholder="Acme, Inc." {...field} />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
                    />

                    <FormField
                      control={newUserForm.control}
                      name="number"
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel>Phone number (already purchased)</FormLabel>
                          <FormControl>
                            <Input placeholder="+15551234567" {...field} />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
                    />

                    <FormField
                      control={newUserForm.control}
                      name="label"
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel>Number label (optional)</FormLabel>
                          <FormControl>
                            <Input placeholder="Main line" {...field} />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
                    />

                    <FormField
                      control={newUserForm.control}
                      name="wireWebhooks"
                      render={({ field }) => (
                        <FormItem className="flex flex-row items-center gap-2 space-y-0">
                          <FormControl>
                            <Checkbox
                              checked={field.value}
                              onCheckedChange={field.onChange}
                            />
                          </FormControl>
                          <FormLabel className="!mt-0">
                            Point this number's Twilio webhooks at this app
                          </FormLabel>
                        </FormItem>
                      )}
                    />

                    {error ? (
                      <div className="text-sm text-red-600">{error}</div>
                    ) : null}

                    <DialogFooter className="gap-2">
                      <Button
                        type="button"
                        variant="outline"
                        onClick={resetAndClose}
                        disabled={isPending}
                      >
                        Cancel
                      </Button>
                      <Button
                        type="submit"
                        disabled={!newUserForm.formState.isValid || isPending}
                      >
                        {isPending && (
                          <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                        )}
                        Create account
                      </Button>
                    </DialogFooter>
                  </form>
                </Form>
              ) : (
                <Form {...existingUserForm}>
                  <form
                    onSubmit={existingUserForm.handleSubmit(
                      onSubmitExistingUser
                    )}
                    className="space-y-5"
                    autoComplete="off"
                  >
                    <FormField
                      control={existingUserForm.control}
                      name="companyName"
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel>Company name</FormLabel>
                          <FormControl>
                            <Input placeholder="Acme, Inc." {...field} />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
                    />

                    <FormField
                      control={existingUserForm.control}
                      name="ownerEmail"
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel>Owner's email</FormLabel>
                          <div className="flex gap-2">
                            <FormControl>
                              <Input
                                type="email"
                                placeholder="owner@example.com"
                                {...field}
                                onChange={(e) => {
                                  field.onChange(e);
                                  setResolvedOwner(null);
                                  setLookupError(null);
                                }}
                              />
                            </FormControl>
                            <Button
                              type="button"
                              variant="secondary"
                              onClick={handleLookup}
                              disabled={lookupPending || !field.value}
                            >
                              {lookupPending ? (
                                <Loader2 className="h-4 w-4 animate-spin" />
                              ) : (
                                <Search className="h-4 w-4" />
                              )}
                            </Button>
                          </div>
                          <FormMessage />

                          {resolvedOwner ? (
                            <div className="flex items-center gap-1.5 text-xs text-emerald-600">
                              <CheckCircle2 className="h-3.5 w-3.5" />
                              Found
                              {resolvedOwner.displayName
                                ? ` ${resolvedOwner.displayName}`
                                : ''}{' '}
                              ({resolvedOwner.uid})
                            </div>
                          ) : lookupError ? (
                            <div className="text-xs text-red-600">
                              {lookupError}
                            </div>
                          ) : (
                            <div className="text-xs text-muted-foreground">
                              Look up the account before creating.
                            </div>
                          )}
                        </FormItem>
                      )}
                    />

                    <FormField
                      control={existingUserForm.control}
                      name="number"
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel>Phone number (already purchased)</FormLabel>
                          <FormControl>
                            <Input placeholder="+15551234567" {...field} />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
                    />

                    <FormField
                      control={existingUserForm.control}
                      name="label"
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel>Number label (optional)</FormLabel>
                          <FormControl>
                            <Input placeholder="Main line" {...field} />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
                    />

                    <FormField
                      control={existingUserForm.control}
                      name="wireWebhooks"
                      render={({ field }) => (
                        <FormItem className="flex flex-row items-center gap-2 space-y-0">
                          <FormControl>
                            <Checkbox
                              checked={field.value}
                              onCheckedChange={field.onChange}
                            />
                          </FormControl>
                          <FormLabel className="!mt-0">
                            Point this number's Twilio webhooks at this app
                          </FormLabel>
                        </FormItem>
                      )}
                    />

                    {error ? (
                      <div className="text-sm text-red-600">{error}</div>
                    ) : null}

                    <DialogFooter className="gap-2">
                      <Button
                        type="button"
                        variant="outline"
                        onClick={resetAndClose}
                        disabled={isPending}
                      >
                        Cancel
                      </Button>
                      <Button
                        type="submit"
                        disabled={
                          !existingUserForm.formState.isValid ||
                          !resolvedOwner ||
                          isPending
                        }
                      >
                        {isPending && (
                          <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                        )}
                        Create account
                      </Button>
                    </DialogFooter>
                  </form>
                </Form>
              )}
            </CardContent>
          </Card>
        )}
      </DialogContent>
    </Dialog>
  );
}
