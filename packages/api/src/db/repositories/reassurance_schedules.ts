...
export const ReassuranceSchedulesRepository = {
  ...
  async getAll(company_id: string): Promise<ReassuranceCallSchedule[]> {
    const res = await pool.query<ReassuranceCallSchedule>(
      `
      SELECT *
      FROM reassurance_call_schedules
  ...
  async delete(id: number, company_id: string): Promise<void> {
    await pool.query(
      `
  ...
  async findActive({
    ...
    const companyClause = company_id ? `AND company_id = $3` : '';
  ...
  async findByContactId(
    ...
    const res = await pool.query<ReassuranceCallSchedule>(
      ...
      WHERE c.id = $1
        ${company_id ? 'AND s.company_id = $2' : ''}
      ORDER BY s.created_at DESC NULLS LAST, s.id DESC
      LIMIT 1
      `,
      company_id ? [contact_id, company_id] : [contact_id]
    );
